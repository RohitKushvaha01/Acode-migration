...
public class SystemBarPlugin extends Service {
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        ...
        host.getActivity().runOnUiThread(this::updateSystemBars);
    }
    ...
    public boolean execute(String action, JSONArray args, Callback callbackContext) throws JSONException {
        if ("setStatusBarVisible".equals(action)) {
            ...
            host.getActivity().runOnUiThread(() -> setStatusBarVisible(visible));
        } else if ("setStatusBarBackgroundColor".equals(action)) {
            host.getActivity().runOnUiThread(() -> setStatusBarBackgroundColor(args));
        } else {
            return false;
        }
    ...
    private void updateRootView(int bgColor) {
        Window window = host.getActivity().getWindow();

        // Set the root view's background color. Works on SDK 36+
        View root = host.getActivity().findViewById(android.R.id.content);
     ...
     */
    private FrameLayout getRootLayout(AppView webView) {
     ...
     * @return View|null
     */
    private View getStatusBarView(AppView webView) {
