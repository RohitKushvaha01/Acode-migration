...
The native APIs are available through `Bridge.exec`, `Bridge.file`, `Bridge.http`, `Bridge.clipboard` and `Bridge.websocket`. Native globals and the `deviceready` event remain available. Extensions that directly use the previous native bridge must update to these names; source modules use ordinary imports.
